import React from 'react'
import RecentItem from '../RecentItem/RecentItem'
import './Profile.css'

function Profile() {
    return (
        <div className='profile'>
            <div className='profile__topLeft'>
                <img src='https://yt3.ggpht.com/ytc/AAUvwnhXNjC8scVdtSnJclwrMOzSVO42dGfuquo_QvOe-w=s88-c-k-c0x00ffffff-no-rj' alt='Star Citizen' />
                <div className='profile__topLeftDetails'>
                    <h1>Star Citizen</h1>
                    <h3>2.8m followers</h3>
                </div>
            </div>
            <div className='profile__topRight'>
                <i class="far fa-heart graybg"></i>
                <i class="far fa-bell graybg"></i>
                <i class="fas fa-ellipsis-v"></i>
            </div>
            <div className='profile__menu'>
                <h2 className='active'>Home</h2>
                <h2>Info</h2>
                <h2>Guide</h2>
                <h2>Videos</h2>
                <h2><i class="fas fa-arrow-up"></i></h2>
                <h2>Chat</h2>
            </div>
            <div className='profile__recent'>
                <h2>Recent broadcasts</h2>
                <div className='profile__recentItems'>
                    <RecentItem 
                        url={'https://www.youtube.com/embed/zMSGlZSL0DE'}
                        title={'Squadron 42: Official Trailer 2018'}
                    />
                    <RecentItem 
                        url={'https://www.youtube.com/embed/_8VFw1F-olQ'}
                        title={'Star Citizen: CitizenCon 2949 - Building a Dynamic Universe'}
                    />
                    <RecentItem 
                        url={'https://www.youtube.com/embed/_aCE7gxQOVY'}
                        title={'Squadron 42: 2019 Visual Teaser'}
                    />
                    <RecentItem 
                        url={'https://www.youtube.com/embed/xWikpyIU_RQ'}
                        title={'Star Citizen: The Reunion - Part 1'}
                    />
                </div>

                <div className='profile__categories'>
                    <h2>Star Citizen's recently streamed Categories</h2>
                    <img src='data:image/png;base64,' />
                    <h3>Videogames</h3>
                </div>
            </div>
        </div>
    )
}

export default Profile
